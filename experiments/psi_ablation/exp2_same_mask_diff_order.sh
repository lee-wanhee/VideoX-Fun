#!/bin/bash
# ==============================================================================
# Experiment 2: Same Masking, Different Order
# - Fixed masking seed (same masks across batches)
# - Random order (different token order in parallel_extract_features each batch)
#
# Usage: bash exp2_same_mask_diff_order.sh <host_address> <node_rank>
# Example: bash exp2_same_mask_diff_order.sh n12 0
# ==============================================================================
...
if [ -f ~/.wandb_api_key ]; then
    ...
    export WANDB_RUN_NAME="exp2_same_mask_diff_order"
...
export OUTPUT_DIR="/scratch/m000063/users/wanhee/VideoX-Fun/output_exp2_same_mask_diff_order_${TIMESTAMP}"

# Create output directory only on master node
if [ "$NODE_RANK" -eq 0 ]; then
    mkdir -p $OUTPUT_DIR
    mkdir -p /scratch/m000063/users/wanhee/VideoX-Fun/.run_sync
    echo "${TIMESTAMP}" > /scratch/m000063/users/wanhee/VideoX-Fun/.run_sync/exp2_timestamp
    echo "Created output dir: $OUTPUT_DIR"
else
    # Worker nodes: wait for master to create timestamp file, then read it
    sleep 5
    if [ -f /scratch/m000063/users/wanhee/VideoX-Fun/.run_sync/exp2_timestamp ]; then
        TIMESTAMP=$(cat /scratch/m000063/users/wanhee/VideoX-Fun/.run_sync/exp2_timestamp)
        export OUTPUT_DIR="/scratch/m000063/users/wanhee/VideoX-Fun/output_exp2_same_mask_diff_order_${TIMESTAMP}"
    fi
...
echo "=============================================="
echo "Experiment 2: Same Masking, Different Order"
echo "=============================================="
