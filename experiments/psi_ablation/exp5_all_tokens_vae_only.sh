#!/bin/bash
# ==============================================================================
# Experiment 5: All Tokens Feature VAE Only (No Projection)
# - Random masking (different masks each batch)
# - Random order (different token order each batch)
# - VAE only mode (NO PSI semantic projection, only VAE-encoded decoded frames)
# - use_all_tokens=True (4x more features per patch - better decoded frames)
#
# This experiment uses parallel_extract_features_all_tokens which extracts
# features for ALL 4 content tokens per patch. However, since psi_vae_only is
# enabled, only the VAE-encoded decoded frames are used (no semantic projection).
# The all_tokens mode gives better decoded frames compared to coarse-only.
#
# Usage: bash exp5_all_tokens_vae_only.sh <host_address> <node_rank>
# Example: bash exp5_all_tokens_vae_only.sh n12 0
# ==============================================================================
...
if [ -f ~/.wandb_api_key ]; then
    ...
    export WANDB_RUN_NAME="exp5_all_tokens_vae_only"
...
export OUTPUT_DIR="/scratch/m000063/users/wanhee/VideoX-Fun/output_exp5_all_tokens_vae_only_${TIMESTAMP}"
...
echo "=============================================="
echo "Experiment 5: All Tokens Feature VAE Only"
echo "=============================================="
...
echo "  - psi_vae_only: True (NO PSI semantic projection)"
echo "  - psi_use_all_tokens: True (better decoded frames)"
