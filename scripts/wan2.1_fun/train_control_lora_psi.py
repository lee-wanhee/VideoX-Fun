...
def main():
            ...
            def save_model_hook(models, weights, output_dir):
                ...
                if accelerator.is_main_process:
                    ...
                    if args.enable_psi_control:
                        ...
                        accelerator.print(f"Saved PSI projection to {psi_projection_save_path}")
            ...
            def save_model_hook(models, weights, output_dir):
                if accelerator.is_main_process:
                    ...
                    if args.enable_psi_control:
                        ...
                        psi_projection_save_path = os.path.join(output_dir, "psi_projection.safetensors")
                        psi_projection_state_dict = {k: v.to(weight_dtype) for k, v in accelerator.unwrap_model(psi_projection).state_dict().items()}
                        save_safetensors(psi_projection_state_dict, psi_projection_save_path, metadata={"format": "pt"})
                        accelerator.print(f"Saved PSI projection to {psi_projection_save_path}")
    ...
    train_dataset = ImageVideoControlDataset(
        ...
        enable_psi_control=args.enable_psi_control
    )
    ...
    if args.enable_psi_control:
        psi_projection.to(accelerator.device if not args.low_vram else "cpu", dtype=weight_dtype)
        psi_control_extractor.to(accelerator.device if not args.low_vram else "cpu", dtype=weight_dtype)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*([transformer3d, psi_projection] if args.enable_psi_control else [transformer3d])):
                ...
                with torch.no_grad():
                    ...
                    if args.train_mode != "control_camera_ref":
                        if args.enable_psi_control:
                            ...
                            # decoded_frames: (B, 2, C, H, W) -> need (B*2, 1, C, H, W) for _batch_encode_vae
                            # _batch_encode_vae expects (B, F, C, H, W) and rearranges internally
                            decoded_frames_flat = decoded_frames.view(B * num_psi_frames, C, H, W).unsqueeze(1)  # (B*2, 1, C, H, W)
                            control_latents_flat = _batch_encode_vae(decoded_frames_flat)  # (B*2, C_latent, 1, H', W')
                ...
                if args.low_vram:
                    ...
                    # PSI models need to be on GPU for training - move them here after VAE is done
                    if args.enable_psi_control:
                        psi_projection.to(accelerator.device)
                        psi_control_extractor.to(accelerator.device)
                ...
                # Move PSI models to CPU after they're used, before transformer forward pass
                if args.low_vram and args.enable_psi_control:
                    psi_projection.to('cpu')
                    psi_control_extractor.to('cpu')
                    torch.cuda.empty_cache()
